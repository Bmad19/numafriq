...
export function ClientAuthPage() {
  ...
  const input = "w-full rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3.5 text-sm text-white placeholder:text-white/25 focus:border-lime/50 focus:outline-none focus:ring-1 focus:ring-lime/30 transition";
  const label = "block text-[11px] font-bold uppercase tracking-widest text-white/40 mb-1.5";

  return (
    <div className="min-h-screen bg-[#08090b] flex flex-col items-center justify-center px-4 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="text-center mb-8">
          ...
          />
          <div className="inline-flex items-center gap-2 rounded-full border border-lime/20 bg-lime/8 px-4 py-2 mb-4" style={{ background: "rgba(163,230,53,0.05)" }}>
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-4 w-4 text-lime">
              <circle cx="8" cy="5" r="3"/><path d="M2 14c0-3.3 2.7-6 6-6s6 2.7 6 6"/>
            </svg>
            <span className="text-xs font-bold uppercase tracking-widest text-lime">Espace Client</span>
          </div>
          ...
          <p className="mt-2 text-sm text-white/40">
            {tab === "login"
              ? "Accédez à votre espace client NUMAFRIQ"
          ...
          {(["login", "register"] as const).map(t => (
            <button key={t} onClick={() => { setTab(t); setError(""); }}
              className={`flex-1 rounded-xl py-2.5 text-sm font-bold transition ${
                tab === t ? "bg-lime text-ink shadow-md" : "text-white/40 hover:text-white"
              }`}>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-7 backdrop-blur-xl shadow-2xl">
          {error && (
            <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
              className="mb-5 flex items-center gap-2 rounded-xl border border-red-500/20 bg-red-500/8 px-4 py-3 text-sm text-red-400" style={{ background: "rgba(239,68,68,0.06)" }}>
              ⚠️ {error}
            </motion.div>
          )}
        ...
        <p className="text-center text-xs text-white/20 mt-6">
          NUMAFRIQ · Votre agence digitale de confiance
        </p>
      </motion.div>
    </div>
  );
