...
const HR_COLORS: Record<string,string> = { conge:"text-lime bg-lime/10 border-lime/20",absence:"text-coral bg-coral/10 border-coral/25",retard:"text-violet bg-violet/10 border-violet/20",prime:"text-violet bg-violet/10 border-violet/20",note:"text-white/60 bg-white/5 border-white/10" };
const HR_STATUS_COLORS: Record<string,string> = { en_attente:"text-violet bg-violet/10 border-violet/20",approuve:"text-lime bg-lime/10 border-lime/20",refuse:"text-coral bg-coral/10 border-coral/25" };
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label:"En attente", value: stats.en_attente, color:"text-violet", icon:"⏳" },
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          className="flex-1 min-w-[180px] rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-white placeholder:text-white/50 focus:border-coral/40 focus:outline-none transition"
        />
      ...
      <div className="rounded-2xl border border-white/[0.08] overflow-hidden">
        {filtered.length === 0 ? (
          <div className="py-16 text-center text-white/48 text-sm">Aucun enregistrement correspondant</div>
        ) : (
                ...
                {filtered.map(r => (
                  <tr key={r.id} className="hover:bg-white/[0.02] transition group">
                    ...
                    <td className="px-5 py-3.5 text-white/60 font-medium whitespace-nowrap">
                      {r.amount ? <span className="text-violet">{r.amount.toLocaleString("fr-FR")} <span className="text-xs text-white/40">FCFA</span></span> : <span className="text-white/48">–</span>}
                    </td>
                    <td className="px-5 py-3.5">
                        ...
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition">
                          {r.status === "en_attente" && <>
                            ...
                            <button onClick={() => approve(r.id, "refuse")}
                              className="rounded-lg bg-coral/10 border border-coral/25 text-coral text-xs px-2.5 py-1 hover:bg-coral/20 transition font-bold">
                              ✗ Refuser
                            </button>
                          </>}
        ...
        {missions.map(m => (
          <div key={m.id} className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-5">
            ...
            <div className="flex items-center justify-between mt-3 text-xs text-white/35">
              ...
              {isAdmin && <button onClick={async () => { if (confirm("Supprimer ?")) { await missionsApi.delete(m.id); load(); } }} className="text-coral hover:text-coral/80">×</button>}
            </div>
          </div>
        ))}
      </div>
...
export function RetoursPage() {
  ...
  const statusColor: Record<string,string> = { nouveau:"text-coral",traite:"text-lime",archive:"text-white/55" };
        ...
        {feedbacks.map(f => (
          <div key={f.id} className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-5">
            ...
            <div className="flex items-center justify-between mt-4">
              <span className="text-xs text-white/55">{new Date(f.created_at).toLocaleDateString("fr-FR")}</span>
...
export function SettingsPage() {
  ...
  const input = "w-full rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white placeholder:text-white/45 focus:border-coral/50 focus:outline-none transition";
  ...
  return (
    <div className="max-w-lg space-y-6">
      ...
      {err && <div className="text-sm text-coral bg-coral/10 border border-coral/25 rounded-xl px-4 py-3">{err}</div>}
      ...
      <div className={section}>
        ...
        <button onClick={changePw} className="w-full rounded-xl bg-violet py-3 text-sm font-bold text-ink hover:brightness-110 transition">Changer le mot de passe</button>
      </div>
    </div>
  );
}
