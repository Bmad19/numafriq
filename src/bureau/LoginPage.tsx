    ...
    <div className="min-h-screen bg-ink flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="text-center mb-10">
          <BrandLogo variant="auth" className="mx-auto mb-5" />
          <h1 className="font-display text-2xl font-bold text-white">Espace Bureau</h1>
          <p className="mt-1 text-sm text-white/40">Accès réservé aux collaborateurs Afrilex Conseil</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                <svg className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-white/48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" /><circle cx="12" cy="7" r="4" />
                </svg>
                <input
                  type="text" value={username} onChange={e => setUsername(e.target.value)}
                  placeholder="votre_identifiant" required autoComplete="username"
                  className="w-full rounded-2xl border border-white/10 bg-white/[0.04] pl-11 pr-4 py-3.5 text-sm text-white placeholder:text-white/45 focus:border-coral/50 focus:outline-none focus:ring-1 focus:ring-coral/30 transition"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-white/10 bg-white/[0.04] pl-11 pr-11 py-3.5 text-sm text-white placeholder:text-white/45 focus:border-coral/50 focus:outline-none focus:ring-1 focus:ring-coral/30 transition"
                />
                <button type="button" onClick={() => setShowPw(v => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/48 hover:text-white/60 transition">
            ...
            {error && (
              <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                className="flex items-center gap-2 rounded-xl bg-coral/10 border border-coral/25 px-4 py-3 text-sm text-coral">
        ...
        <p className="text-center text-xs text-white/48 mt-6">
          Accès réservé aux collaborateurs Afrilex Conseil · Version {new Date().getFullYear()}
        </p>
      </motion.div>
    </div>
  );
